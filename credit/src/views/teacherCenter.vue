    ...
    <el-card class="profile-card" shadow="never">
      ...
      >
        ...
        <el-descriptions-item>
          ...
          <el-tag type="info" class="id-tag">{{ teacher.id || '--' }}</el-tag>
        </el-descriptions-item>
        ...
        <el-descriptions-item>
          ...
          <div class="id-tag">{{ teacher.name || '未填写' }}</div>
        </el-descriptions-item>
        ...
        <el-descriptions-item>
          ...
          <div class="id-tag">
            <el-link :href="`tel:${teacher.phone}`" type="primary">
              {{ teacher.phone || '未填写' }}
            </el-link>
          </div>
        </el-descriptions-item>
...
export default {
  ...
  methods: {
    loadTeacherInfo() {
      const storedData = window.localStorage.getItem('teacher');
      if (storedData) {
        try {
          this.teacher = JSON.parse(storedData);
        } catch (e) {
          console.error('信息解析失败:', e);
          this.$message.error('个人信息加载失败');
        }
      } else {
        this.$message.warning('未找到信息');
      }
    },
    refreshData() {
      this.loadTeacherInfo();
